/* uqMatObj.H
  ...
  object-oriented interface for OptiVec matrix functions of type "uquad"
...
#define mta  matrix<uquad>&
#define vta  vector<uquad>&
#define ta   uquad&

#define mt   matrix<uquad>
#define vt   vector<uquad>
#define t    uquad
#define tMatrix uqMatrix
#define tVector uqVector

#define tMatObj uqMatObj
...
//
		...
		{	if((ht > 0) && (len > 0))
			{	mx_matr=MUQ_matrix(ht,len); vx_m0=mx_matr[0];
		...
		{	if(!ht || !len) //size == 0
			...
			else
			{
				ui_ht=ht; ui_len=len; ui_size=((ui)ht)*len;
				if(!fill) //fill == 0.0
				{	mx_matr=MUQ_matrix0(ht,len); vx_m0=mx_matr[0];}
				else
				{	mx_matr=MUQ_matrix(ht,len);  vx_m0=mx_matr[0];
					VUQ_equC(vx_m0,ui_size,fill);}
			}
		}

		matrix(cmta init)
		{	if(init.ui_size)
			{	mx_matr= MUQ_matrix(init.ui_ht, init.ui_len); vx_m0=mx_matr[0];
				ui_ht=init.ui_ht; ui_len=init.ui_len; ui_size=init.ui_size;
				VUQ_equV(vx_m0, init.vx_m0, ui_size);
			} else
			{ mx_matr=0; vx_m0=0; ui_ht=0; ui_len=0; ui_size=0;}
		}
...
//
		...
		{	if(!X.ui_size) dealloc(); //can set size to zero
			...
			{	if(mx_matr) M_free(mx_matr);
				mx_matr= MUQ_matrix(X.ui_ht, X.ui_len); vx_m0=mx_matr[0];
...
//
		...
		{	setSize(matr.ui_ht, matr.ui_len);
			enforceSize();
			VUQ_equV(vx_m0, matr.vx_m0, ui_size);
			return *this;}

		mta operator=(cta scalar)
		{	enforceSize();
			VUQ_equC(vx_m0, ui_size, scalar);
			return *this;}
...
//
		...
		void _matrix(cuint ht, cuint len) //underscore necessary to avoid confusing with constructor
		{
			dealloc();
			ui_size=((ui)ht)*len;
			if(ui_size){ ui_ht=ht;ui_len=len;mx_matr= MUQ_matrix(ht,len);vx_m0=mx_matr[0];}
		}
		...
		void _matrix0(cuint ht, cuint len) //underscore necessary to avoid confusing with constructor
		{
			dealloc();
			ui_size=((ui)ht)*len;
			if(ui_size){ ui_ht=ht;ui_len=len;mx_matr= MUQ_matrix0(ht,len);vx_m0=mx_matr[0];}
		}
...
//
		...
		{
			if(ui_ht<=m || ui_len<=n) throw OptiVec_SmallSizeErr();
			return MUQPelement(vx_m0,ui_ht,ui_len,m,n);
		}

		t element(cuint m, cuint n) const
		{
			if(ui_ht<=m || ui_len<=n) throw OptiVec_SmallSizeErr();
			return MUQelement(vx_m0,ui_ht,ui_len,m,n);
		}

		void getElement(t *y, cuint m, cuint n)
		{
			if(ui_ht<=m || ui_len<=n) throw OptiVec_SmallSizeErr();
			return MUQgetElement(y,vx_m0,ui_ht,ui_len,m,n);
		}

		void setElement(cuint m, cuint n, cta C)
		{
			if(ui_ht<=m || ui_len<=n) throw OptiVec_SmallSizeErr();
			MUQsetElement(vx_m0,ui_ht,ui_len,m,n,C);
		}
...
//
		...
		void QItoUQ(const qiMatObj& X);

		//defined by including ubMatObj.h
		void UBtoUQ(const ubMatObj& X);

		//defined by including usMatObj.h
		void UStoUQ(const usMatObj& X);

		//defined by including uMatObj.h
		void UtoUQ(const uMatObj& X);

		//defined by including ulMatObj.h
		void ULtoUQ(const ulMatObj& X);

		//defined by including fMatObj.h
		int roundtoUQ(const fMatObj& X);
		int floortoUQ(const fMatObj& X);
		int ceiltoUQ(const fMatObj& X);
		int choptoUQ(const fMatObj& X);
		int trunctoUQ(const fMatObj& X)
		{
			return choptoUQ(X);
		}

		//defined by including dMatObj.h
		int roundtoUQ(const dMatObj& X);
		int floortoUQ(const dMatObj& X);
		int ceiltoUQ(const dMatObj& X);
		int choptoUQ(const dMatObj& X);
		int trunctoUQ(const dMatObj& X)
		{
			return choptoUQ(X);
		}
		...
		int roundtoUQ(const eMatObj& X);
		int floortoUQ(const eMatObj& X);
		int ceiltoUQ(const eMatObj& X);
		int choptoUQ(const eMatObj& X);
		int trunctoUQ(const eMatObj& X)
		{
			return choptoUQ(X);
		}
#endif //__BORLANDC__
...
//
		void submatrix(cmta MSrce, cuint firstRowInCol, cuint sampInCol, cuint firstColInRow, cuint sampInRow )
		{
			...
			MUQsubmatrix(vx_m0, ui_ht, ui_len, MSrce.vx_m0, MSrce.ui_ht, MSrce.ui_len,
		...
		void submatrix_equM(cuint firstRowInCol, cuint sampInCol, cuint firstColInRow, cuint sampInRow, cmta MSrce)
		{
			...
			MUQsubmatrix_equM(vx_m0, ui_ht, ui_len,
			    firstRowInCol, sampInCol, firstColInRow, sampInRow,
		...
		void equMblock(cmta MSrce, cuint firstRowInCol, cuint firstColInRow )
		{
			...
			MUQequMblock(vx_m0, ui_ht, ui_len, MSrce.vx_m0, MSrce.ui_ht, MSrce.ui_len,
		...
		void equMblockT(cmta MSrce, cuint firstRowInCol, cuint firstColInRow )
		{
			...
			MUQequMblockT(vx_m0, ui_ht, ui_len, MSrce.vx_m0, MSrce.ui_ht, MSrce.ui_len,
			             firstRowInCol, firstColInRow );
		}
		...
		void block_equM(cuint firstRowInCol, cuint firstColInRow, cmta MSrce )
		{
			...
			MUQblock_equM(vx_m0, ui_ht, ui_len, firstRowInCol, firstColInRow,
			             MSrce.vx_m0, MSrce.ui_ht, MSrce.ui_len );
		}
		...
		void block_equMT(cuint firstRowInCol, cuint firstColInRow, cmta MSrce )
		{
			...
			MUQblock_equMT(vx_m0, ui_ht, ui_len, firstRowInCol, firstColInRow,
			              MSrce.vx_m0, MSrce.ui_ht, MSrce.ui_len );
		}
...
//
		...
		void Row_delete(cmta MA, cuint iRow)
		{
			...
			{	ui_size -= ui_len;  // don't reallocate, as row pointers remain intact
				MUQRow_delete(vx_m0, vx_m0, ui_ht--, ui_len, iRow);
			}
			else
			{	enforceSize(MA.ui_ht-1, MA.ui_len);
				MUQRow_delete(vx_m0, MA.vx_m0, MA.ui_ht, MA.ui_len, iRow);
			}
		}
		...
		void Col_delete(cmta MA, cuint iCol)
		{
			...
			if( mx_matr == MA.mx_matr ) // overwrite MA?
			{
				tMatrix tmp = MUQ_matrix( ui_ht, ui_len-1 );
				MUQCol_delete(tmp[0], MA.vx_m0, ui_ht, ui_len--, iCol);
				M_free( mx_matr );
				vx_m0 = tmp[0];
				ui_size -= ui_ht;
				mx_matr = tmp;
			...
			else
			{
				...
				MUQCol_delete(vx_m0, MA.vx_m0, ui_ht, ui_len, iCol);
			}
		}
		...
		void Row_insert(cmta MA, cuint iRow, cvta X)
		{
			...
			if( mx_matr == MA.mx_matr ) // overwrite MA?
			{
				tMatrix tmp = MUQ_matrix( ++ui_ht, ui_len );
				MUQRow_insert(tmp[0], MA.vx_m0, ui_ht, ui_len, iRow, X.vx_vect);
				M_free( mx_matr );
				vx_m0 = tmp[0];
				ui_size += ui_len;
				mx_matr = tmp;
			...
			{
				enforceSize(MA.ui_ht+1, MA.ui_len);
				MUQRow_insert(vx_m0, MA.vx_m0, ui_ht, ui_len, iRow, X.vx_vect);
			}
		}
		...
		void Col_insert(cmta MA, cuint iCol, cvta X)
		{
			...
			{
				tMatrix tmp = MUQ_matrix( ui_ht, ++ui_len );
				MUQCol_insert(tmp[0], MA.vx_m0, ui_ht, ui_len, iCol, X.vx_vect);
				M_free( mx_matr );
				vx_m0 = tmp[0];
				ui_size += ui_ht;
				mx_matr = tmp;
			...
			{
				enforceSize(MA.ui_ht, MA.ui_len+1);
				MUQCol_insert(vx_m0, MA.vx_m0, ui_ht, ui_len, iCol, X.vx_vect);
			}
		}
...
//(some of these functions belong to uqVecObj.h)

		t Dia_max() const
		{
			if( !ui_len || (ui_len != ui_ht) )throw OptiVec_SizeErr();
			return MUQDia_max(vx_m0, ui_len);
		}

		t Dia_min() const
		{
			if( !ui_len || (ui_len != ui_ht) )throw OptiVec_SizeErr();
			return MUQDia_min(vx_m0, ui_len);
		}

		void Rows_rotate(const ssize_t pos)
		{
			enforceSize();
			MUQRows_rotate(vx_m0, ui_ht, ui_len, pos);
		}

		void Cols_rotate(const ssize_t pos)
		{
			enforceSize();
			MUQCols_rotate(vx_m0, ui_ht, ui_len, pos);
		}

		void Rows_rotate_buf(const ssize_t pos, cmta MBuf)
		{
			enforceSize();
			MUQRows_rotate_buf(vx_m0, ui_ht, ui_len, pos, MBuf.vx_m0);
		}

		void Cols_rotate_buf(const ssize_t pos, cmta MBuf)
		{
			enforceSize();
			MUQCols_rotate_buf(vx_m0, ui_ht, ui_len, pos, MBuf.vx_m0);
		}

		void Rows_reflect()
		{
			enforceSize();
			MUQRows_reflect(vx_m0, ui_ht, ui_len);
		}

		void Cols_reflect()
		{
			enforceSize();
			MUQCols_reflect(vx_m0, ui_ht, ui_len);
		}

		void Rows_rev()
		{
			enforceSize();
			MUQRows_rev(vx_m0, ui_ht, ui_len);
		}

		void Cols_rev()
		{
			enforceSize();
			MUQCols_rev(vx_m0, ui_ht, ui_len);
		}
...
//
	...
	void Rows_distribution(cvta Limits, cmta MA, int mode=0)
	{
		...
		return MUQRows_distribution(vx_m0,Limits.vx_vect,ui_size,MA.vx_m0,MA.ui_ht,MA.ui_len,mode);
	}

	void Cols_distribution(cvta Limits, cmta MA, int mode=0)
	{
		if( !MA.ui_ht || !MA.ui_len || ui_len != MA.ui_len ) throw OptiVec_SizeErr();
		return MUQCols_distribution(vx_m0,Limits.vx_vect,ui_size,MA.vx_m0,MA.ui_ht,MA.ui_len,mode);
	}
		...
		//defined by including uMatObj.h
		void Rows_distribution(const uVecObj& Limits, const uMatObj& MA, int mode);
		void Cols_distribution(const uVecObj& Limits, const uMatObj& MA, int mode);
...
//
		void transpose(cmta MA)
		{
			...
			MUQtranspose(vx_m0, MA.vx_m0, ui_ht, ui_len);
		}

		void rotate90(cmta MA)
		{
			enforceSize(MA.ui_len, MA.ui_ht);
			MUQrotate90(vx_m0, MA.vx_m0, ui_ht, ui_len);
		}

		void rotate180(cmta MA)
		{
			enforceSize(MA.ui_len, MA.ui_ht);
			MUQrotate180(vx_m0, MA.vx_m0, ui_ht, ui_len);
		}

		void rotate270(cmta MA)
		{
			enforceSize(MA.ui_len, MA.ui_ht);
			MUQrotate270(vx_m0, MA.vx_m0, ui_ht, ui_len);
		}
...
/////////////////////////////////////////////////////////////////////////////////////////
//Data fitting
// All of the fitting functions themselves belong to uqVecObj.
...
//
		...
		{
			enforceSize();
			MUQfprint(stream,vx_m0,ui_ht, ui_len,linewidth);
		}

		void cprint() const
		{
			enforceSize();
			MUQcprint(vx_m0,ui_ht,ui_len);
		}
		...
		{
			enforceSize();
			MUQwrite(stream,vx_m0,ui_ht,ui_len);
		}

		void read(FILE* stream)
		{
			enforceSize();
			MUQread(vx_m0,ui_ht,ui_len,stream);
		}
		...
		void store(FILE* stream) const
		{
			...
			VUQ_store(stream,vx_m0,ui_size);
		}
		...
		void recall(FILE* stream)
		{
			...
			VUQ_recall(vx_m0,ui_size,stream);
		}
			...
			{
				enforceSize(X.ui_size,Y.ui_size);
				MUQxyzAutoDensityMap( X.vx_vect, Y.vx_vect, vx_m0, ui_ht, ui_len,
														mincol, maxcol );
			}
			...
			{
				enforceSize(X.ui_size,Y.ui_size);
				MUQxyzDataDensityMap( X.vx_vect, Y.vx_vect, vx_m0, ui_ht, ui_len);
			}

			void zAutoDensityMap( COLORREF mincol, COLORREF maxcol ) const
			{
				enforceSize();
				MUQzAutoDensityMap( vx_m0, ui_ht, ui_len, mincol, maxcol );
			}

			void zDataDensityMap() const
			{
				enforceSize();
				MUQzDataDensityMap( vx_m0, ui_ht, ui_len);
			}
